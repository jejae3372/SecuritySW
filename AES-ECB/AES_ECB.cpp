...
byte ct[16];       //암호문을 저장하는 변수
//(1) pt xor prev_iv --> [AES] --> ct, ct --> prev_iv 
//(2) ct ----> prev_iv
...
//=[AES_Enc_ECB() 함수를 수정하여 완성] ===================
void AES_Dec_ECB(const char* fileCT, byte key[16], const char* filePT) {
    ifstream fin;
    ofstream fout;
    char ch;
...
void AES_Dec_CBC(const char* fileCT, byte key[16], byte IV[16], const char* filePT) {
    ...
    // (2) buffer(ct) --> prev_iv
    Copy_state(IV, prev_iv); //초기설정
    for (int i = 0; i < num_block - 1; i++) { // 마지막 블록 직전까지 
        fin.read((char*)buffer, 16);
        AES32_EqDecrypt(buffer, rk, pt); // AES 복호화
        Xor_state(pt, prev_iv);
        fout.write((char*)pt, 16);
        Copy_state(buffer, prev_iv); //(2)
    }
