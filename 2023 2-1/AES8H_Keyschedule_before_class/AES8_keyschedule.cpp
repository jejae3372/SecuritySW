...
// (1) AES32비트 테이블 추가: Te0[]~Te4[], Td0[]~Td4[]
...
void AES8_EqInvKey(byte rk[11][16], byte eqrk[11][16]) {
    ...
    for (int i = 1; i < 10; i++) {  // rk[1], rk[2], ... , rk[9]에만 적용한다.
        InvMixColumns(eqrk[i]);
    }
}
...
//=====
// 바이트 b[16] --> 워드 state[4]
//inline 함수 호출 시 스택에 옮기는 작업없이 바로 그 자리에 코드를 실행하는 것과 같다
// -> 스택에 옮기지 않아 고속화
...
// (2) 키스케줄 함수(32bit)추가
/*
    키스케줄에서 호출하는 내부함수 - u32 RotWord(u32 w32)
    키스케줄에서 호출하는 내부함수 - u32 SubWord(u32 w32)
    암호화 키스케줄(32bit) - void AES32_Enc_KeySchedule(byte k[16], u32 rk[11][4])
    복호화 키스케줄(32bit) - void AES32_Dec_KeySchedule(byte k[16], u32 rk[11][4])
    키스케줄(8bit용) - void AES8_KeySchedule(byte k[16], byte rk[11][16])
*/
...
//=====
// AES32 암호화 키스케줄
//k[16] --> rk[11] = [rk[0], rk[1], rk[2], rk[3], .... , rk[10]
//                         rk[i] = [ rk[i][0], rk[i][1], rk[i][2], rk[i][3] ]
...
//=====
// AES32 복호화 키스케줄 (주의:1라운드 ~ 9라운드에 InvMixCol을 적용한다)
// 함수내 코드에서 32비트 테이블의 활용을 이해하자!
...
void AES8_KeySchedule(byte k[16], byte rk[11][16]) {
    u32 rk32[11][4]; // 8bit 라운드키 저장

    //[1] 32비트 키스케줄 호출    
    AES32_Enc_KeySchedule(k, rk32);

    //[2] u32[4] --> byte[16]으로 저장
    for (int i = 0; i < 11; i++) {
