...
// AES의 Sbox 만들기 --> sbox[] 저정
// x --> w=x^(-1) ---> y = Aw+b : y = S[x]
void Get_AES_Sbox(byte sbox[256]) {
    byte temp;
    // 0의 역원 0^(-1) = 0으로 간주한다
    sbox[0] = AES_Affine(0); // A*0+b
    for (int x = 1; x < 256; x++) {
        temp = GF_inv(x);  // x^(-1)
...
// ISbox : sbox의 역함수
// y = S(x) : y = A*x^(-1) + b
// x = IS(y) : A*x^(-1) = y + b ==> x^(-1) = A^(-1)*(y+b) ==> x = [ A^(-1)*(y+b) ]^(-1)
...
void AES_Encrypt(byte PT[16], byte RK[11][16], byte CT[16]) {
    //RK[11][16] = { RK[0][], RK[1][], ... , RK[10][] } //16 생략 가능
    byte state[16];     //암호화 중간 단계를 저장

    for (int i = 0; i < 16; i++) {
        state[i] = PT[i];
    }

    AddRoundKey(state, RK[0]);      //state에 내용이 업데이트

    //입력, 출력을 다른 메모리에 저장 
    // out_state = AddRoundKey(in_state, RK[0]);

    for (int r = 1; r <= 9; r++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, RK[r]);  // RK만 다르게 적용한다.
        //RK[r] = {RK[r][0], RK[r][1], ... ,RK[r][15]} 
    }
    
    //마지막 Round는 MixColumns 없이
    SubBytes(state);
    ShiftRows(state);
...
void Run_AES8_test() {
    ...
    // ciphertext = "69c4e0d86a7b0430d8cdb78070b4c55a" => 나와야 하는 암호문
