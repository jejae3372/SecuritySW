//==============================
/*
    AES 32비트 구현을 위한 테이블 생성
    암호화 테이블: Te0[], Te1[], Te2[], Te3[], Te4[]  (완성됨)
    [실습] 복호화 테이블: Td0[], Td1[], Td2[], Td3[], Td4[]  (미완성)
*/
//==============================
#include <iostream>
#include "AES8.h"
#include "GF256.h"

typedef unsigned char byte;
typedef unsigned int u32;

static byte Sbox[256] = {
     };

static byte ISbox[256] = {
     };


//=====
// AES32 암호화용 테이블 5개 인쇄 : Te0, Te1, Te2, Te3, Te4
/*
  Te0[x] = [ 02*x, x, x, 03*x ]
  Te1[x] = [ 03*x, 02*x, x, x ]  // Te0[]를 오른쪽으로 한바이트 씩 회전
  Te2[x] = [ x, 03*x, 02*x, x ]
  Te3[x] = [ x, x, 03*x, 02*x ]
  Te4[x] = [ x, x, x, x ] // 10라운드(MixColumn 없는) 전용 테이블
 */
void AES32_Enc_Table_generation() {

    u32 Te0[256], Te1[256], Te2[256], Te3[256], Te4[256];
    byte sx;
    u32 value;

    for (int i = 0; i < 256; i++) {
        sx = Sbox[i];  // 02*x
        value = (xtime(sx) << 24) | sx << 16 | sx << 8 | xtime(sx) ^ sx; 
        Te0[i] = value;   // Te0[x] = [ 02*x, x, x, 03*x ]
        Te1[i] = (Te0[i] >> 8) | (Te0[i] << 24);
        Te2[i] = (Te1[i] >> 8) | (Te1[i] << 24);
        Te3[i] = (Te2[i] >> 8) | (Te2[i] << 24);
        Te4[i] = (sx << 24) | (sx << 16) | (sx << 8) | sx;
    }

    printf("== AES32 Encryption Table ==\n\n");

    printf("Te0[] =\n");
    for (int i = 0; i < 256; i++) {
        printf("0x%08x, ", Te0[i]);
        if ((i % 4) == 3) printf("\n");
    }
    printf("Te1[] =\n");
    for (int i = 0; i < 256; i++) {
        printf("0x%08x, ", Te1[i]);
        if ((i % 4) == 3) printf("\n");
    }
    printf("Te2[] =\n");
    for (int i = 0; i < 256; i++) {
        printf("0x%08x, ", Te2[i]);
        if ((i % 4) == 3) printf("\n");
    }
    printf("Te3[] =\n");
    for (int i = 0; i < 256; i++) {
        printf("0x%08x, ", Te3[i]);
        if ((i % 4) == 3) printf("\n");
    }
    printf("Te4[] =\n");
    for (int i = 0; i < 256; i++) {
        printf("0x%08x, ", Te4[i]);
        if ((i % 4) == 3) printf("\n");
    }
    printf("\n");
}

//=====
// AES32 복호화용 테이블 5개 인쇄 : Td0, Td1, Td2, Td3, Td4
/*
  Td0[x] = [ 0e*x, 09*x, 0d*x, 0b*x ]
  Td1[x] = [ 0b*x, 0e*x, 09*x, 0d*x ]  // Td0[]를 오른쪽으로 한바이트 씩 회전
  Td2[x] = [ 0d*x, 0b*x, 0e*x, 09*x ]
  Td3[x] = [ 09*x, 0d*x, 0b*x, 0e*x ]
  Td4[x] = [ x, x, x, x ] // 10라운드(MixColumn 없는) 전용 테이블
 */
void AES32_Dec_Table_generation() {

    u32 Td0[256], Td1[256], Td2[256], Td3[256], Td4[256];
    byte sx, sx02, sx04, sx08, sx0e, sx0b, sx0d, sx09;
    u32 value;
    // [실습] - 복화화용 테이블 만들기
    /*
    * 
    */



    printf("== AES32 Decryption Table ==\n\n");

    printf("Td0[] =\n");
    for (int i = 0; i < 256; i++) {
        printf("0x%08x, ", Td0[i]);
        if ((i % 4) == 3) printf("\n");
    }
    printf("Td1[] =\n");
    for (int i = 0; i < 256; i++) {
        printf("0x%08x, ", Td1[i]);
        if ((i % 4) == 3) printf("\n");
    }
    printf("Td2[] =\n");
    for (int i = 0; i < 256; i++) {
        printf("0x%08x, ", Td2[i]);
        if ((i % 4) == 3) printf("\n");
    }
    printf("Td3[] =\n");
    for (int i = 0; i < 256; i++) {
        printf("0x%08x, ", Td3[i]);
        if ((i % 4) == 3) printf("\n");
    }
    printf("Td4[] =\n");
    for (int i = 0; i < 256; i++) {
        printf("0x%08x, ", Td4[i]);
        if ((i % 4) == 3) printf("\n");
    }
    printf("\n");
}


int main() {
    AES32_Enc_Table_generation();
    AES32_Dec_Table_generation();
}
